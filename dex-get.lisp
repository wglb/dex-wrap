...
(defstruct (dexans (:type list) :named)
  body status-code headers uri host http-stream must-close status-text fol-de-rol)

(defun dex-good (dexa)
  (equal 200 (dexans-status-code dexa)))
  ...
  (let* ((rv nil)
	...
	(handler-case
		  ...
		  (multiple-value-bind (body status-code headers uri http-stream must-close status-text)
			...
			(setf rv 
				  (make-dexans  :body body  :status-code status-code :headers headers :uri uri 
								:http-stream http-stream :must-close must-close  :status-text status-text :host host ))))
	  (quri.error:uri-invalid-port(e)
		(xlogf "~e for ~a: bad port found " e fetch-this )
		(setf rv (make-dexans :body '(:not-html :ssl :uri-port-error) :status-code 500 :status-text :uri-error)))
	  (SB-SYS:DEADLINE-TIMEOUT (e)
		(xlogf "~e for ~a " e fetch-this)
		(setf rv (make-dexans :body '(:not-html :ssl failure) :status-code 500 :status-text "website-timeout")))
	  ...
	  (USOCKET:CONNECTION-REFUSED-ERROR (er)
		...
		(setf rv (make-dexans :body '(:not-html :ssl failure) :status-code 500 :status-text :connection-refused)))
	  (CL+SSL::SSL-ERROR (hah)
		(xlogf "dg:cl+ssl::ssl-error: busted http request for ~a~%~a" fetch-this hah)
		(let ((ans (make-dexans :body '(:not-html :ssl failure) :status-code 500 :status-text "ssl error")))
		  ans)) 
	  ...
	  (http-request-failed (j)
		...
		(xlogf "dg: http-request-failed for ~s ~%body type ~s" 
			   fetch-this (type-of j))
		(setf rv (make-dexans :body (response-body j) 
							  :status-code (response-status j)
							  :status-text "Bad request" 
							  :headers (response-headers j))))
